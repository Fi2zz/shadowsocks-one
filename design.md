...
- 滚动折叠是阈值 36pt + 迟滞的状态机（方向极值跟随、橡皮筋不跟进、
...
- 2026-09-03 结构修正（用户点破形变机理）：大条与胶囊改为**叠放 + 中段窄窗口
  交替**——大条前 80% 行程缩放、0.5→0.85 淡出；胶囊 0.5→0.85 淡入、
  0.45→1 长到位（时间轴曲线集中在 `BrowserChromeMetrics`，含单测），
  替代全行程线性交叉。
- 2026-09-03 稳定性修正：日志取证发现 WebKit 在加载/布局期间会发出大幅
  contentOffset 跳变（实测连续 -62/-168 再 +477），曾令状态机自行翻转振荡
  （小滚 8pt 却折叠）。修正：① 状态机只吃用户手势驱动的滚动
  （`isDragging || isDecelerating`），布局调整与程序化补偿一律不理——
  滚动自动化钩子 `SSBrowserScrollY` 相应只验证布局、不再驱动折叠，
  新增 `-SSBrowserFoldProgress <0..1>` 直接设定形变进度供视觉验证；
  ② progress 门槛与翻转对齐（offsetY > 0 且未入稳定区），消除
  "展开态却全折叠视觉"的状态脱节。
