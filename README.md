...
> 现状：域名白名单/代理名单均已生效——TCP 建连前按目的 IP 在 `DNSCache` 反查域名（DNS 应答缓存 + 隧道启动时白名单预热）。名单与开关在 App 内「导入与分流」页维护，下次连接生效。CN IP 库内置 17mon/china_ip_list（约 7.4k 条 IPv4 CIDR），App 内「国内 IP 库」区块可从下载地址更新（默认官方 GitHub 地址，可改），下载版优先于内置版；由「国内 IP 直连」开关控制。
