...
> 注意：App Group 与 Bundle ID 已替换为正式值（`group.com.fitz.lifts`、`com.fits.socks.one*`）；keychain service（`com.example.ShadowsocksOne.shared`）仍是占位值，需要时连同两个 target 的 `keychain-access-groups` 一起替换。
